                             ...
                             decoder_t &theta)
{
    ...
    decoder_t tuples;
    tuples.intrinsic_GF.resize(sz_bub);
    tuples.intrinsic_LLR.resize(sz_bub);

    uint16_t i1, i2;
    for (size_t i = 0; i < sz_bub; i++)
    {
        i1 = Bubb_Indicator[0][i];
        i2 = Bubb_Indicator[1][i];
        tuples.intrinsic_GF[i] = ADDDEC[a_gf[i1]][b_gf[i2]];
        tuples.intrinsic_LLR[i] = a[i1] + b[i2];
    }
    ...
    decoder_t unsrt;
    unsrt.intrinsic_LLR.assign(q, std::numeric_limits<softdata_t>::max());
    unsrt.intrinsic_GF.resize(q);
    iota(unsrt.intrinsic_GF.begin(), unsrt.intrinsic_GF.end(), 0);
    ...
    for (size_t i = 0; i < sz_bub; i++)
        if (tuples.intrinsic_LLR[i] < unsrt.intrinsic_LLR[tuples.intrinsic_GF[i]])
            unsrt.intrinsic_LLR[tuples.intrinsic_GF[i]] = tuples.intrinsic_LLR[i];

    tuples_sorter(unsrt, nm, q, theta);
}
